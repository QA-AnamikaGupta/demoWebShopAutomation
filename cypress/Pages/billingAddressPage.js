class BillingAddressPage{
    ...
    // Elements object to store methods that retrieve page elements
      elements={
          ...
          enterFirstName:()=>cy.get('#BillingNewAddress_FirstName'),
          enterLastName:()=>cy.get('#BillingNewAddress_LastName'),
          enterEmail:()=>cy.get('#BillingNewAddress_Email'),
     ...
     // Method to fill the billing address form for GUEST User with provided details
     fillTheBillingAddressFormForGuestORNewUser(firstname,lastname,emailid,city,addressline1,zipcode,phonenumber)
      {
          this.elements.clickCheckoutAsGuestButton().click()
          this.elements.enterFirstName().type(firstname)
          this.elements.enterLastName().type(lastname)
          this.elements.enterEmail().type(emailid)
